function [psi_ref, theta_ref, y_e, z_e, alpha_c_hat, beta_c_hat,d] = ...
    my_ALOS3D(x,y,z,h,wpt,params)
% 三维的ALOS法，得到航向和俯仰的参考角度
...
%,
% Inputs:
%   (x,y,z):  craft North-East-Down positions (m) 航行器的北-东-地位置
%   Delta_h:  positive look-ahead distance, horizontal plane (m) 前视距离（平面）
%   Delta_v:  positive look-ahead distance, vertical plane (m) 前视距离（垂直面）
%   gamma_h:  positive parameter adapation gain, horizontal plane 自适应增益（平面）
%   gamma_v:  positive parameter adapation gain, vertical plane 自适应增益（垂直面）
%   M_theta:  maximum parameter: abs(alpha_c) < M_theta, abs(beta_c) < M_theta 最大参数？
%   h:        sampling time (s) 采样时间
%   R_switch: go to next waypoint when the inside the circle of radius R_switch (m) 切换半径
%   wpt.pos.x = [x1, x2,..,xn]' array of waypoints expressed in NED (m) 航点数组
%   wpt.pos.y = [y1, y2,..,yn]'
%   wpt.pos.z = [z1, z2,..,zn]'
%
...
%    alpha_c_hat: estimate of alpha_c (rad) % TODO 参数含义
%    beta_c_hat:  estimate of beta_c (rad) % TODO 参数含义
%
% Reference:
...
pi_h = atan2( (yk_next-yk) , (xk_next-xk) ); % 这个方向正好反了
pi_v = atan2( -(zk_next-zk) , sqrt( (xk_next-xk)^2 + (yk_next-yk)^2 ) );
% disp([xk yk xk_next yk_next zk_next pi_h pi_v]);
