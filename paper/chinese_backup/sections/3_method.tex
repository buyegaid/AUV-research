...
EG-UCCO的核心思想是利用CFD预标定的动力学模型进行一步前向速度预测，将预测值与DVL实测速度的残差通过灵敏度分析映射为海流估计的梯度方向，并在激励条件满足时进行更新。图\ref{fig:architecture}给出了EG-UCCO的整体架构。
...
\begin{equation}
    \text{更新条件}: \quad \lambda_{\min}(\bm{W}_c) > \mu_{\text{gate}}
    \label{eq:gate}
\end{equation}

其中$\mu_{\text{gate}}=10^{-8}$为门控阈值。设置较低的门控阈值确保正常机动时不会误阻断更新，同时在高噪声、弱激励时为估计提供保护。门控机制将"稳态不可辨识"这一理论缺陷转化为实用特性：在需要时才更新，不需要时自动维持。

\textbf{GM时间传播}：当激励不足时，海流估计按一阶Gauss-Markov过程进行时间传播：
...
其中$\tau_c=100$s为海流相关时间常数，$\bar{\bm{c}}$为海流均值（通常取零或先验值）。GM传播确保在DVL短暂失效或长时直航段期间，海流估计不会发散。

\subsection{梯度更新律}

当激励条件满足时，采用梯度下降更新海流估计：
...
其中$\bm{e}_{\nu} = \bm{\nu}_{k+1}^{\text{meas}} - \hat{\bm{\nu}}_{k+1|k}$为速度预测新息（仅取surge和sway分量），$\gamma$为有效增益，$\kappa$为自适应正则化系数。更新量被限制在单步最大变化量$\Delta c_{\max}=0.01$ m/s以内，防止单步过冲。
