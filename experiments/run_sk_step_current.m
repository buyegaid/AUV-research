function [rmse_phase1, rmse_phase2] = run_sk_step_current(omega0, usePIESO)
% 阶跃海流测试：前100s有流(Vc=0.8)，后100s无流(Vc=0)
% 验证PI-ESO在扰动消失后的恢复速度
...
% 分段RMSE
ep1 = hist_pos(1:N_half, :) - target';
rmse_phase1 = sqrt(mean(sum(ep1.^2, 2)));
ep2 = hist_pos(N_half+1:end, :) - target';
rmse_phase2 = sqrt(mean(sum(ep2.^2, 2)));
end
