function pos_rmse = run_sk_tauc(omega0, seed, usePIESO, tau_c_val)
% 定点控制测试：可变omega0、种子、tau_c（同时设置真实海流和ESO假设的tau_c）
...
params.pieso.tau_c = tau_c_val;  % ESO使用的tau_c

h = 0.01; T = 200; t = 0:h:T; N = length(t);

rng(seed);
gm.Vc_mean = 0.8; gm.betaVc_mean = pi/4;
gm.sigma_Vc = 0.08; gm.tau_c = tau_c_val;  % 真实海流tau_c与ESO匹配
