...
- 4 通道 SMC 控制器：`src/matlab/controller/xhy/smc_surge_xhy.m`（纵荡）、`smc_yaw_xhy`（转艏）、`smc_pitch_xhy`（俯仰）、`smc_heave_xhy`（垂荡）
- 深度控制双模式：`DepthMode=1`（俯仰角控深）vs `DepthMode=2`（直接 Z 力控深）
- Station-keeping 定点控制测试：`apps/run_sk.m`、`apps/run_sk_omega0.m`、`apps/run_sk_tauc.m` 等
...
**REMUS 平台 ESO 补偿方式（`src/matlab/controller/remus/SMCheading.m`）：**
```matlab
